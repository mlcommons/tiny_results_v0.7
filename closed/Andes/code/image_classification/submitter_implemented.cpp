...
#include "tensorflow/lite/micro/tools/make/targets/andesevb/ae350/bsp/perf_counter.h"
#include "tensorflow/lite/micro/tools/make/targets/andesevb/ae350/bsp/ae350/ae350.h"
...
#define printf(arg...) do { sprintf((char *)gstr, ##arg); uart_puts((char *)gstr); } while (0)

#define timestamp_in_usec ((unsigned long)(((unsigned long long )rdmcycle() * (unsigned long long) 1000000) / (unsigned long long) CPUFREQ))
...
void th_load_tensor() {
  ...
  bytes = ee_get_buffer(reinterpret_cast<uint8_t *>(input_quantized),
...
void th_results() {
  ...
  for (size_t i = 0; i < kCategoryCount; i++) {
	...
	th_printf("%0.3f", (double) converted);
    if (i < (nresults - 1)) {
      th_printf(",");
    }
  }
...
void th_final_initialize(void) {
  ...
  static tflite::MicroModelRunner<int8_t, int8_t, 7> model_runner(
         pretrainedResnet_quant_tflite, resolver, tensor_arena_local, kTensorArenaSize);
  runner = &model_runner;
}
...
char th_getchar() { return (char) uart_getc(); }
