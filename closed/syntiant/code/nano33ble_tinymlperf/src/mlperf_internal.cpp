...
extern char g_dblog[];  // print dbg text to here instead of over serial port
extern int g_dblg_pos;
...
void ee_serial_callback(char c) {
  if (c == EE_CMD_TERMINATOR) {
    ...
    g_cmd_pos = 0;
  } else if (c != char(255)) { // 255 = -1 = return value when no char availble
...
void ee_serial_command_parser_callback(char *p_command) {
  ...
  if (strncmp(tok, EE_CMD_NAME, EE_CMD_SIZE) == 0) {
    th_printf(EE_MSG_NAME, EE_DEVICE_NAME, TH_VENDOR_NAME_STRING); // EE_MSG_NAME="m-name-%s-[%s]\r\n"
...
void ee_infer(size_t n, size_t n_warmup) {
  // th_printf("jhdbg: in ee_infer[1]. g_buff_pos = %d\r\n", g_buff_pos);
...
arg_claimed_t ee_buffer_parse(char *p_command) {
  ...
  // th_printf("dbg: in ee_buffer_parse; prob db\r\n");
  // g_dblg_pos += sprintf(g_dblog+g_dblg_pos, "dbg: bufparse;\r\n");
  p_next = strtok(NULL, EE_CMD_DELIMITER);
  ...
  if (p_next == NULL) {
    ...
    // g_dblg_pos += sprintf(g_dblog+g_dblg_pos, "dbg: should be in db load block\r\n");
    size_t numbytes;
    char test[3];
    long res;
    ...
    test[2] = 0;
    // th_printf("dbg: copy nb=%d; pos=%d\r\n", numbytes, g_buff_pos);
    // g_dblg_pos += sprintf(g_dblog+g_dblg_pos, "dbg: cp; nb=%d; pos=%d\r\n", numbytes, g_buff_pos);
    // numbytes is actually the number of hex digits.  A command like "db 5152"
    // gives numbytes=4, but loads 4 hex digits / 2 actual bytes.
...
size_t ee_get_buffer(uint8_t* buffer, size_t max_len) {
  // th_printf("ee_gb: max_len = %d; g_buff_pos = %d\r\n", max_len, g_buff_pos);
