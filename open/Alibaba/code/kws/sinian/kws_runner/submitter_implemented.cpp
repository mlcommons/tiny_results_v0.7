...
void th_load_tensor() {
  int8_t data_int8[kInputSize];
  size_t bytes = ee_get_buffer(reinterpret_cast<uint8_t *>(data_int8),
  ...
  float zero_point = 83.0;
  float scale = 0.5847;

  for (int i = 0; i < kInputSize; i++) {
     finput[i] = (float(data_int8[i])-zero_point)*scale ;
  }
