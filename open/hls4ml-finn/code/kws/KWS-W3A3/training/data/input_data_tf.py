""" Model definitions for simple speech recognition.
    Taken from the tiny MLperf 0.5 closed training examples: https://github.com/mlcommons/tiny_results_v0.5/blob/main/closed/reference/code/training/keyword_spotting/get_dataset.py
"""
...
def get_mfcc_from_py_speech(sliced_foreground, tf_window_size_samples, tf_sample_rate, tf_window_size_ms, tf_window_stride_ms, tf_dct_coefficient_count):
    nfft = int(2 ** np.ceil(np.log2(tf_window_size_samples)))
    inp_array = sliced_foreground
    mfcc_feat_py = mfcc(inp_array, tf_sample_rate,
                        ...
                        lowfreq=20.0,
                        highfreq=4000.0,
                        winfunc=sc_signal.windows.hann,
                        appendEnergy=False,
                        preemph=0.,
                        ceplifter=0.,
                        )
    # The windowing is a bit off for some reason, so we have to adjust a bit here.
    mfcc_feat_py = tf.convert_to_tensor(mfcc_feat_py[:-1, :], dtype=tf.float32)
    return mfcc_feat_py
...
def get_preprocess_audio_func(model_settings, is_training=False, background_data=[]):
    def prepare_processing_graph(next_element):
        ...
        """
        desired_samples = model_settings['tf_desired_samples']
        background_frequency = model_settings['tf_background_frequency']
        background_volume_range_ = model_settings['tf_background_volume_range_']

        wav_decoder = tf.cast(next_element['audio'], tf.float32)
        if model_settings['tf_feature_type'] != "td_samples":
        ...
        if model_settings['tf_feature_type'] == 'mfcc':
            stfts = tf.signal.stft(sliced_foreground, frame_length=model_settings['tf_window_size_samples'],
                                   frame_step=model_settings['tf_window_stride_samples'], fft_length=None,
            ...
            mfccs = tf.signal.mfccs_from_log_mel_spectrograms(log_mel_spectrograms)[...,
                    :model_settings['tf_dct_coefficient_count']]
            mfccs = tf.reshape(mfccs,
                               [model_settings['tf_spectrogram_length'], model_settings['tf_dct_coefficient_count'], 1])
            next_element['audio'] = mfccs
            # next_element['label'] = tf.one_hot(next_element['label'],12)

        elif model_settings['tf_feature_type'] == 'mfcc_py':
            inp_list = [sliced_foreground, model_settings['tf_window_size_samples'],
                        model_settings['tf_sample_rate'], model_settings['tf_window_size_ms'],
                        model_settings['tf_window_stride_ms'], model_settings['tf_dct_coefficient_count']]
            mfcc_feat_py = tf.numpy_function(get_mfcc_from_py_speech, inp_list, tf.float32)
            mfccs = tf.reshape(mfcc_feat_py, [model_settings['tf_spectrogram_length'], model_settings['tf_dct_coefficient_count'], 1])
            next_element['audio'] = mfccs

        elif model_settings['tf_feature_type'] == 'lfbe':
            # apply preemphasis
            preemphasis_coef = 1 - 2 ** -5
            power_offset = 52
            num_mel_bins = model_settings['tf_dct_coefficient_count']
            ...
            powspec = (1 / model_settings['tf_window_size_samples']) * tf.square(magspec)
            ...
            lower_edge_hertz, upper_edge_hertz = 0.0, model_settings['tf_sample_rate'] / 2.0
            linear_to_mel_weight_matrix = (
                tf.signal.linear_to_mel_weight_matrix(
                    num_mel_bins=num_mel_bins,
                    num_spectrogram_bins=num_spectrogram_bins,
                    sample_rate=model_settings['tf_sample_rate'],
        ...
        elif model_settings['tf_feature_type'] == 'td_samples':
...
def get_training_data(data_args, get_waves=False, val_cal_subset=False):
    #label_count = 12
    #background_frequency = Flags.background_frequency
    #background_volume_range_ = Flags.background_volume
    #model_settings = models.prepare_model_settings(label_count, data_args)

    # Calculate parameters, which are derived from the data_args
    data_args['tf_desired_samples'] = int(data_args['tf_sample_rate'] * data_args['tf_clip_duration_ms'] / 1000)
    data_args['tf_window_size_samples'] = int(data_args['tf_sample_rate'] * data_args['tf_window_size_ms'] / 1000)
    data_args['tf_window_stride_samples'] = int(data_args['tf_sample_rate'] * data_args['tf_window_stride_ms'] / 1000)
    length_minus_window = (data_args['tf_desired_samples'] - data_args['tf_window_size_samples'])
    if length_minus_window < 0:
        data_args['tf_spectrogram_length'] = 0
    else:
        data_args['tf_spectrogram_length'] = 1 + int(length_minus_window / data_args['tf_window_stride_samples'])

    bg_path = data_args['tf_bg_path']
    ...
    (ds_train, ds_test, ds_val), ds_info = tfds.load('speech_commands', split=splits,
                                                     data_dir=data_args['tf_data_dir'], with_info=True, download=False)
    ...
    if data_args['tf_num_train_samples'] != -1:
        ds_train = ds_train.take(data_args['tf_num_train_samples'])
    if data_args['tf_num_val_samples'] != -1:
        ds_val = ds_val.take(data_args['tf_num_val_samples'])
    if data_args['tf_num_test_samples'] != -1:
        ds_test = ds_test.take(data_args['tf_num_test_samples'])
    ...
    if get_waves:
        ...
        ds_train = ds_train.map(get_preprocess_audio_func(data_args, is_training=True,
                                                          background_data=background_data),
                                num_parallel_calls=tf.data.experimental.AUTOTUNE)
        ds_test = ds_test.map(get_preprocess_audio_func(data_args, is_training=False,
                                                        background_data=background_data),
                              num_parallel_calls=tf.data.experimental.AUTOTUNE)
        ds_val = ds_val.map(get_preprocess_audio_func(data_args, is_training=False,
                                                      background_data=background_data),
                            num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    # ds_train = ds_train.batch(data_args.batch_size)
    # ds_test = ds_test.batch(data_args.batch_size)
    # ds_val = ds_val.batch(data_args.batch_size)
    # ds_train = ds_train.batch(1)
    # ds_test = ds_test.batch(1)
    # ds_val = ds_val.batch(1)

    # Return numpy generators, no need to do this in batches,
    # just return everyting and the batch generation will be done by PyTorch.
